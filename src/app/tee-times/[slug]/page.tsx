...
export async function generateMetadata({
  ...
  const canonicalUrl = `${SITE_URL}/tee-times/${params.slug}`;
  const description = `Browse tee times in ${cityTitle}, Florida, explore public golf courses, and quickly access official booking links with TeeTimeUs.`;

  return {
    title: `${cityTitle} Tee Times | TeeTimeUs`,
    description,
    alternates: {
...
export default async function TeeTimesCityPage({
  params,
}: {
  params: { slug: string };
}) {
        ...
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          ...
          >
            ...
            <div style={{ marginTop: 8, color: ui.muted }}>
              We couldn’t find a tee times page for this location yet.
            </div>
  ...
  const pageUrl = `${SITE_URL}/tee-times/${params.slug}`;
  ...
  const itemListElements = courses.map((course, index) => {
    const courseUrl =
      course.tee_time_url || course.website_url || `${SITE_URL}/cities/${params.slug}`;
  ...
  const pageSchema = {
    ...
    "@type": "CollectionPage",
    name: `${matchedCity} Tee Times`,
    url: pageUrl,
    description: `Browse tee times in ${matchedCity}, Florida, explore public golf courses, and quickly access official booking links with TeeTimeUs.`,
    isPartOf: {
      "@type": "WebSite",
      name: "TeeTimeUs",
      url: SITE_URL,
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "56px 16px 70px" }}>
        ...
        >
          ...
          >
            <div>
              ...
              >
                Browse public golf courses in {matchedCity}, Florida and quickly
                access official booking links in one place. TeeTimeUs makes it
                easier to compare golf options and book your next round.
              </div>
            </div>

            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <Link
                href={`/cities/${params.slug}`}
                style={{
                  display: "inline-flex",
                  alignItems: "center",
                  justifyContent: "center",
        ...
        >
          <div style={{ fontSize: 22, fontWeight: 950, letterSpacing: "-0.01em" }}>
            Book Tee Times in {matchedCity}, Florida
          </div>
          ...
          >
            {matchedCity} gives golfers a convenient place to explore public golf
            courses, compare options, and access official booking links. Whether
            you are looking for a weekday round, a weekend tee time, or a new
            course to try, TeeTimeUs helps make the process easier.
          </div>
          ...
          >
            This tee times page is designed to target golfers searching for golf in
            {matchedCity}, while also helping visitors discover nearby cities and
            additional Florida golf options. Explore the active course listings
            below and click through to book directly with each course.
          </div>
        </div>
        ...
        >
          <div style={{ fontSize: 20, fontWeight: 900 }}>
            Explore More Florida Tee Times
          </div>

          <div style={{ marginTop: 10, color: ui.muted, fontSize: 14 }}>
            Browse tee times in nearby cities and surrounding golf markets.
          </div>
          ...
          >
            {nearbyCities.map((city) => (
              <Link
                key={city}
                href={`/tee-times/${cityToSlug(city)}`}
                style={{
                  padding: "8px 12px",
                  borderRadius: 999,
                  border: `1px solid ${ui.border}`,
        ...
        <div style={{ marginTop: 18, color: ui.muted, fontSize: 12 }}>
          Dynamic tee times page • URL: /tee-times/{params.slug}
        </div>
      </div>
    </div>
  );
